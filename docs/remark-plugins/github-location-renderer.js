    ...
    return function(tree) {
        visit(tree, (node, index, parent) => {
            ...
            for (let i = index - 1; i >= 0; i--) {
                if (
                    (parent.children[i].tagName === 'div'
                        && parent.children[i].properties.className.includes('object-signature'))
                    || (parent.children[i].type === 'heading' && parent.children[i].depth < node.depth)
                ) {
            ...
            if (prev_heading) {
                ...
                const container_node = h('a', { class: 'code-location-text', href }, [github_icon, {
                    type: 'text',
                    value: 'Code location'
                }]);
